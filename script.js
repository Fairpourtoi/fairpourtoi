...
try {
  ...
  // [UPDATED] Subscribe strip below the Events cards (Shop the Edit card
  // no longer has its own form — the action moved here to cover the
  // shop, brands, and event announcements together) — reuses the same
  // addToMailchimp() function already used everywhere else on the page.
  function sendEventsSubscribeForm(event) {
    event.preventDefault();
    const status = document.getElementById('events-subscribe-status');
    const btn = document.getElementById('events-subscribe-btn');
    const emailField = document.getElementById('events-subscribe-email');
    const email = emailField.value;
    ...
    addToMailchimp(email, (alreadySubscribed) => {
      ...
      status.textContent = alreadySubscribed
        ...
        : 'Thank you for subscribing 🥂!';
      btn.textContent = 'Subscribed';
      setTimeout(() => { status.textContent = ''; btn.textContent = 'Subscribe'; btn.disabled = false; }, 8000);
    }, () => {
      status.textContent = 'Something went wrong. Please try again.';
      btn.textContent = 'Subscribe';
      btn.disabled = false;
